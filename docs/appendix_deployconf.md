...
Deployment of the ETL scripts is coordinated through the [transportation-data-deploy](https://github.com/cityofaustin/atd-data-deploy) project. The script runs from within a Docker container built with the [transportation-data-publishing](https://github.com/cityofaustin/atd-data-publishing]) project (with added dependencies as noted below), and in these examples retrieves transformation code from directories mounted on the host system. This document describes the configurations that are currently running on the two servers that run ETL tasks.
